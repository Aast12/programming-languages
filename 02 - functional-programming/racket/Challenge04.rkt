...
(define (prime-aux n k)
	(cond 
		((> k (sqrt n)) #t)
		((= (modulo n k ) 0) #f)
		(else (prime-aux n (+ k 2)))
	)
)
...
(define (sumDigits n)
  	(if (< n 10)
		n
		(+ (sumDigits (quotient n 10)) (modulo n 10))
	)
)
...
(define (xor lstA lstB)
	(if (null? lstA)
		'()
		(let ((a (car lstA)) (b (car lstB)))
			(append 
				(list (or (and a (not b)) (and b (not a))))
